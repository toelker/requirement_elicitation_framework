...
class LLMInterface:
    ...
    def _finalize_system_description(self):
        ...
        system_prompt = (
            "You are a requirements engineer. Please rewrite this description using the NABC framework (Need, Approach, Benefit, Competition):\n\n"
            f"{prompt}"
        )
    ...
    def _ask_stakeholder_prompt(self):
        stakeholder_prompt = (
            "Based on the system description, please list potential stakeholders in the format 'Name: Description'. Only provide the list."
        )
    ...
    def _request_requirements(self):
        if self.current_stakeholder_idx < len(self.stakeholder_list):
            ...
            prompt = (
                f"For '{current_stakeholder}: {self.stakeholders[current_stakeholder]}', please list their "
                "requirements in the format 'Requirement Name: Description'. Only provide the list."
            )
    ...
    def _request_persona(self):
        current_stakeholder = self.stakeholder_list[self.current_stakeholder_idx]
        persona_prompt = (
                f"Create a detailed persona for '{current_stakeholder}' based on these requirements:\n" +
